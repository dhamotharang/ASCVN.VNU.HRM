...
import { FormLoaiDuAnComponent } from './form-loai-du-an/form-loai-du-an.component';


@Component({
    selector: 'app-loai-du-an',
    templateUrl: './loai-du-an.component.html',
    styleUrls: ['./loai-du-an.component.scss']
})
export class LoaiDuAnComponent extends BaseLaboratoryCatalogComponent<ILoaiDuAn> implements OnInit, OnDestroy {
    constructor(
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('SCI.LDA.LOAI_DU_AN'),
            content: FormLoaiDuAnComponent,
            width: 850,
            top: 10,
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOnOk: () => {
                    this.apiService
                        .delete(UrlConstant.API.DM_LOAI_DU_AN, body)
     ...
     * @param dataItem
     */
    removeHandler(dataItem: ILoaiDuAn) {
        this.selectionIds = [];
    ...
    protected loadItems() {
        this.isLoading = true;
        this.gridView$ = this.apiService.post(UrlConstant.API.DM_LOAI_DU_AN + '/GetList', this.queryOptions).pipe(
            map(res => {
