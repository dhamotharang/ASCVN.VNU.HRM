...
import { IVaiTro } from '../_models/ptn.model';
import { FormVaiTroComponent } from './form-vai-tro/form-vai-tro.component';
...
export class VaiTroComponent extends BaseLaboratoryCatalogComponent<IVaiTro> implements OnInit, OnDestroy {
    ...
    protected showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('PTN.VT.VAI_TRO'),
            content: FormVaiTroComponent,
            width: 850,
            top: 10,
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOnOk: () => {
                    this.apiService
                        .delete(UrlConstant.API.DM_VAI_TRO, body)
    ...
    * @param dataItem
    */
    removeHandler(dataItem: IVaiTro) {
        this.selectionIds = [];
    ...
    */
    protected loadItems() {
        this.isLoading = true;
        this.gridView$ = this.apiService.post(UrlConstant.API.DM_VAI_TRO + '/GetList', this.queryOptions).pipe(
            map(res => {
