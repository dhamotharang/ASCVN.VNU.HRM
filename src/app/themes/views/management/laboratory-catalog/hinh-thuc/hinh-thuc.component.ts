...
import { IHinhThuc } from '../_models/ptn.model';
import { FormHinhThucComponent } from './form-hinh-thuc/form-hinh-thuc.component';
...
export class HinhThucComponent extends BaseLaboratoryCatalogComponent<IHinhThuc> implements OnInit, OnDestroy {
    constructor(
    ...
    protected showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('SCI.HT.HINH_THUC'),
            content: FormHinhThucComponent,
            width: 850,
            top: 10,
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOnOk: () => {
                    this.apiService
                        .delete(UrlConstant.API.DM_HINH_THUC, body)
     ...
     * @param dataItem
     */
    removeHandler(dataItem: IHinhThuc) {
        this.selectionIds = [];
    ...
    protected loadItems() {
        this.isLoading = true;
        this.gridView$ = this.apiService.post(UrlConstant.API.DM_HINH_THUC + '/GetList', this.queryOptions).pipe(
            map(res => {
