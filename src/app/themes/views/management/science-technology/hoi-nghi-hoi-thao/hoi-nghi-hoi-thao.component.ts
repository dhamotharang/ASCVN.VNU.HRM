...
import { FormHoiNghiHoiThaoComponent } from './form-hoi-nghi-hoi-thao/form-hoi-nghi-hoi-thao.component';
...
@Component({
    selector: 'app-hoi-nghi-hoi-thao',
    templateUrl: './hoi-nghi-hoi-thao.component.html',
    styleUrls: ['./hoi-nghi-hoi-thao.component.scss'],
})
export class HoiNghiHoiThaoComponent extends BaseScienceTechnologyComponent<IHoiNghiHoiThao> implements OnInit, OnDestroy {
    ...
    private get extendQueryOptions() {
        return {
            ...
            idLoaiHoiThaoHoiNghis: this.extendModelSearch.idLoaiHoiThaoHoiNghis,
    ...
    ngOnInit() {
        ...
        this.rolesExtends.isDuyet =  this.isHasPermission(ListRoleOption.KHCN_0005);
        if (this.idPhongThiNghiem) {
            this.extendModelSearch.idPhongThiNghiem = this.idPhongThiNghiem;
        }
        /*
            0 - Cá nhân
            1 - Quản lý
        ...
        if (e.selectedRows.length > 0) {
            ...
            listChon.map(x => {
                if(this.isQuanLy || (x.dataItem.createdById == this.userId && x.dataItem.idTrangThaiDuyet !== this.trangThaiDuyet)){
                    ...
                    this.selectionNames.push(x.dataItem.tenHoiThaoHoiNghi);
                }
            });
        }
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('SCI.HNHT.TITLE_HOI_NGHI_HOI_THAO'),
            content: FormHoiNghiHoiThaoComponent,
            width: 1200,
            top: 100,
    ...
    removeHandler(dataItem: IHoiNghiHoiThao) {
        this.selectionIds = [];
    ...
    onDuyetKhongDuyet() {
        if (this.selectionIds.length > 0) {
            this.title = this.translate.get('SCI.DUYET_HN');
            this.opened = true;
            const windowRef = this.windowService.open({
                title: this.title,
            ...
            param.action = this.action;
            param.model = this.model;
            param.idManHinh = EKHCN.HOI_NGHI;
    ...
    onExportExcel() {
        this.fileService.exportFile(this.url + '/ExportExcel', this.extendQueryOptions, 'hoi-nghi-hoi-thao');
    }
}
