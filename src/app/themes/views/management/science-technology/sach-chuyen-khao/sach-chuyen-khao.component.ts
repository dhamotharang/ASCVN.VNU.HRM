...
import { FormSachChuyenKhaoComponent } from './form-sach-chuyen-khao/form-sach-chuyen-khao.component';
...
@Component({
    selector: 'app-sach-chuyen-khao',
    templateUrl: './sach-chuyen-khao.component.html',
    styleUrls: ['./sach-chuyen-khao.component.scss'],
})
export class SachChuyenKhaoComponent extends BaseScienceTechnologyComponent<ISachChuyenKhao> implements OnInit, OnDestroy {
    ...
    private get extendQueryOptions() {
        return {
            ...
            namXuatBans: this.extendModelSearch.namXuatBans,
            isTiengViet: this.extendModelSearch.isTiengViet,
    ...
    ngOnInit() {
        ...
        this.rolesExtends.isDuyet =  this.isHasPermission(ListRoleOption.KHCN_0008);
        ...
        if (e.selectedRows.length > 0) {
            ...
            listChon.map(x => {
                if(this.isQuanLy || (x.dataItem.createdById == this.userId && x.dataItem.idTrangThaiDuyet !== this.trangThaiDuyet)){
                    ...
                    this.selectionNames.push(x.dataItem.maSachChuyenKhao);
                }
            });
        }
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('SCI.SCK.TITLE_SACH_CHUYEN_KHAO'),
            content: FormSachChuyenKhaoComponent,
            width: 1200,
            top: 100,
    ...
    removeHandler(dataItem: ISachChuyenKhao) {
        this.selectionIds = [];
    ...
    onDuyetKhongDuyet() {
        if (this.selectionIds.length > 0) {
            this.title = this.translate.get('SCI.DUYET_SCK');
            this.opened = true;
            const windowRef = this.windowService.open({
                title: this.title,
            ...
            param.action = this.action;
            param.model = this.model;
            param.idManHinh = EKHCN.SACH_CHUYEN_KHAO;
    ...
    onExportExcel() {
        this.fileService.exportFile(this.url + '/ExportExcel', this.extendQueryOptions, 'sach-chuyen-khao');
    }
}
