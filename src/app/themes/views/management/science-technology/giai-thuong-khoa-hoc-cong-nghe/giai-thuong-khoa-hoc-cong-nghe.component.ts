...
import { FormGiaiThuongKhoaHocCongNgheComponent } from './form-giai-thuong-khoa-hoc-cong-nghe/form-giai-thuong-khoa-hoc-cong-nghe.component';

@Component({
    selector: 'app-giai-thuong-khoa-hoc-cong-nghe',
    templateUrl: './giai-thuong-khoa-hoc-cong-nghe.component.html',
    styleUrls: ['./giai-thuong-khoa-hoc-cong-nghe.component.scss'],
})
export class GiaiThuongKhoaHocCongNgheComponent extends BaseScienceTechnologyComponent<IGiaiThuongKHCN> implements OnInit, OnDestroy {
    ...
    private get extendQueryOptions() {
        return {
            ...
            idLoaiGiaiThuongs: this.extendModelSearch.idLoaiGiaiThuongs,
            idThuHangGiaiThuongs: this.extendModelSearch.idThuHangGiaiThuongs,
            idCoQuans: this.extendModelSearch.idCoQuans.length > 0 ? this.extendModelSearch.idCoQuans.map(x => Number(x)) : [],
            ngayQuyetDinh: this.extendModelSearch.ngayQuyetDinh,
    ...
    ngOnInit() {
        ...
        this.rolesExtends.isDuyet =  this.isHasPermission(ListRoleOption.KHCN_0004);
        ...
        if (e.selectedRows.length > 0) {
            ...
            listChon.map(x => {
                if(this.isQuanLy || (x.dataItem.createdById == this.userId && x.dataItem.idTrangThaiDuyet !== this.trangThaiDuyet)){
                    ...
                    this.selectionNames.push(x.dataItem.tenGiaiThuongKHCN);
                }
            });
        }
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('SCI.GTKHCN.TITLE_GIAI_THUONG_KHCN'),
            content: FormGiaiThuongKhoaHocCongNgheComponent,
            width: 1200,
            top: 100,
    ...
    onDuyetKhongDuyet() {
        if (this.selectionIds.length > 0) {
            this.title = this.translate.get('SCI.DUYET_GT');
            this.opened = true;
            const windowRef = this.windowService.open({
                title: this.title,
            ...
            param.action = this.action;
            param.model = this.model;
            param.idManHinh = EKHCN.GIAI_THUONG;
 ...
 onExportExcel() {
    this.fileService.exportFile(this.url + '/ExportExcel', this.extendQueryOptions, 'giai-thuong-khoa-hoc-cong-nghe');
}
}
