...
import { FormBaoCaoKhoaHocComponent } from './form-bao-cao-khoa-hoc/form-bao-cao-khoa-hoc.component';
...
@Component({
    selector: 'app-bao-cao-khoa-hoc',
    templateUrl: './bao-cao-khoa-hoc.component.html',
    styleUrls: ['./bao-cao-khoa-hoc.component.scss'],
})
export class BaoCaoKhoaHocComponent extends BaseScienceTechnologyComponent<IBaoCaoKhoaHoc> implements OnInit, OnDestroy {
    ...
    private get extendQueryOptions() {
        return {
            ...
            ngayBaoCao: this.extendModelSearch.ngayBaoCao,
            idLoaiBaoCaos: this.extendModelSearch.idLoaiBaoCaos,
            idDeTais: this.extendModelSearch.idDeTais,
            idHoiThaos: this.extendModelSearch.idHoiThaos,
    ...
    ngOnInit() {
        ...
        this.rolesExtends.isDuyet =  this.isHasPermission(ListRoleOption.KHCN_0002);
        ...
        if (e.selectedRows.length > 0) {
            ...
            listChon.map(x => {
                if(this.isQuanLy || (x.dataItem.createdById == this.userId && x.dataItem.idTrangThaiDuyet !== this.trangThaiDuyet)){
                    ...
                    this.selectionNames.push(x.dataItem.maBaoCaoKhoaHoc);
                }
            });
        }
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('SCI.SCK.TITLE_QUAN_LY_BAO_CAO_KH'),
            content: FormBaoCaoKhoaHocComponent,
            width: 1200,
            top: 100,
    ...
    removeHandler(dataItem: IBaoCaoKhoaHoc) {
        this.selectionIds = [];
    ...
    onDuyetKhongDuyet() {
        if (this.selectionIds.length > 0) {
            this.title = this.translate.get('SCI.DUYET_BC');
            this.opened = true;
            const windowRef = this.windowService.open({
                title: this.title,
    ...
    onExportExcel() {
        this.fileService.exportFile(this.url + '/ExportExcel', this.extendQueryOptions, 'bao-cao-khoa-hoc');
    }
}
