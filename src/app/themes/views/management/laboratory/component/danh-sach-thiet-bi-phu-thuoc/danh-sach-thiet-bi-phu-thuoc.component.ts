...
import { FormThietBiPhuThuocComponent } from './form-thiet-bi-phu-thuoc/form-thiet-bi-phu-thuoc.component';

@Component({
    selector: 'app-danh-sach-thiet-bi-phu-thuoc',
    templateUrl: './danh-sach-thiet-bi-phu-thuoc.component.html',
    styleUrls: ['./danh-sach-thiet-bi-phu-thuoc.component.scss'],
})
export class DanhSachThietBiPhuThuocComponent extends BaseCheckPermission implements OnInit, OnDestroy {
    @ViewChild(TooltipDirective) public tooltipDir: TooltipDirective;
    @Input() idThietBi: number;
    private destroyed$ = new Subject();
    gridView$: Observable<GridDataResult>;
    private model: IThietBiPhuThuoc;
    ...
    private loadItems() {
        this.isLoading = true;
        this.gridView$ = this.apiService.post(UrlConstant.API.THIET_BI_PHU_THUOC + '/GetDanhSachThietBiPhuThuoc', this.queryOptions, true).pipe(
            map(res => {
    ...
    addHandler(flag: boolean) {
        ...
        this.title = this.translate.get("PTN.TITLE.C_THIET_BI_PHU_THUOC");
    ...
    editHandler(dataItem, flag: boolean) {
        this.model = dataItem;
        this.title = this.translate.get("PTN.TITLE.M_THIET_BI_PHU_THUOC");
    ...
    openForm(flag: boolean) {
        ...
        const windowRef = this.windowService.open({
            title: this.title,
            content: FormThietBiPhuThuocComponent,
            width: 850,
            top: 10,
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOkType: 'danger',
                nzOnOk: () => {
                    const remove$ = this.apiService.delete(UrlConstant.API.THIET_BI_PHU_THUOC, body).pipe(takeUntil(this.destroyed$));
                    remove$.subscribe(() => {
                        this.selectionIds = [];
