...
export class IconProjectComponent implements OnInit {
  ...
  ngOnInit() {
    this.data = Array.from(IconDictionary, ([name, value]) => ({ name, value }));
    this.icons = this.data;
    this.searchControl.valueChanges.pipe(debounceTime(500)).subscribe(value => {
        let valueUPC =  value.toUpperCase();
        let arraySearch = [];
        this.data.forEach(element => {
            if(element.name.toUpperCase().indexOf(valueUPC) > -1 ){
                arraySearch.push(element);
            }
        });
        this.icons = arraySearch;
    });
  }
  ...
  private loadItems() {
      this.loading = true;
      this.apiService.read(UrlConstant.API.HRM_SETTING_BIEU_MAU + '/List', this.queryOptions).pipe(
