...
import { FormQuaTrinhCongDoanComponent } from './form-qua-trinh-cong-doan/form-qua-trinh-cong-doan.component';
...
@Component({
    selector: 'app-qua-trinh-cong-doan',
    templateUrl: './qua-trinh-cong-doan.component.html',
    styleUrls: ['./qua-trinh-cong-doan.component.scss'],
})
export class QuaTrinhCongDoanComponent extends BaseHumanResourceComponent<IQuaTrinhCongDoan> implements OnInit, OnDestroy {
    isShowInfo = false;
    constructor(
    ...
    ngOnInit() {
        super.ngOnInit();
        // this.isShowInfo = (isShowTabHRM(hrmkey.QuaTrinhCongDoan) && isDuyetLyLichNhanSu) || !isDuyetLyLichNhanSu;
    }
    ...
    removeHandler(dataItem: IQuaTrinhCongDoan) {
        this.selectionIds = [];
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOnOk: () => {
                    this.apiService
                        .delete(UrlConstant.API.HRM_NS_QUA_TRINH_CONG_DOAN, body)
    ...
    onApprove(flag: boolean) {
        const body: IDuyetThongTinNhanSuModel = {
            ...
            idsNhanSuQuaTrinhCongDoan: this.selectionIds,
        };
        this.opened = true;
    ...
    protected showFormCreateOrUpdate() {
        this.opened = true;
        const windowRef = this.windowService.open({
            title: 'Quá trình Công Đoàn',
            content: FormQuaTrinhCongDoanComponent,
            width: 700,
            top: 10,
            autoFocusedElement: 'body',
        });
    ...
    protected loadItems() {
        this.loading = true;
        this.gridView$ = this.apiService.read(UrlConstant.API.HRM_NS_QUA_TRINH_CONG_DOAN, this.queryOptions).pipe(
