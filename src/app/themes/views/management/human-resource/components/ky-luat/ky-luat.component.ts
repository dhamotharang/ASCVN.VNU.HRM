...
export class KyLuatComponent extends BaseHumanResourceComponent<INhanSuKyLuat> implements OnInit, OnDestroy {
    isShowInfo = false;
    constructor(
    ...
    ngOnInit() {
        super.ngOnInit();
        // this.isShowInfo = (isShowTabHRM(hrmkey.KyLuat) && isDuyetLyLichNhanSu) || !isDuyetLyLichNhanSu;
        this.isShowInfo = this.isShowTabHRM(HRM_KEY.KyLuat) &&
    ...
    removeHandler(dataItem: INhanSuKyLuat) {
        this.selectionIds = [];
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOnOk: () => {
                    this.apiService
                        .delete(UrlConstant.API.HRM_NS_KY_LUAT, body)
    ...
    onApprove(flag: boolean) {
        const body: IDuyetThongTinNhanSuModel = {
            ...
            idsNhanSuKyLuat: this.selectionIds,
        };
        this.opened = true;
    ...
    protected loadItems() {
        this.loading = true;
        this.gridView$ = this.apiService.read(UrlConstant.API.HRM_NS_KY_LUAT, this.queryOptions).pipe(
