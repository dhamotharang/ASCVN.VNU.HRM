...
import { IQuaTrinhDang } from '@themes/views/management/human-resource/_models/thong-tin-doan-dang.model';
...
import { FormQuaTrinhDangComponent } from '@themes/views/management/human-resource/components/qua-trinh-dang/form-qua-trinh-dang/form-qua-trinh-dang.component';
...
@Component({
    selector: 'app-qua-trinh-dang',
    templateUrl: './qua-trinh-dang.component.html',
    styleUrls: ['./qua-trinh-dang.component.scss'],
})
export class QuaTrinhDangComponent extends BaseHumanResourceComponent<IQuaTrinhDang> implements OnInit, OnDestroy {
    isShowInfo = false;
    constructor(
    ...
    ngOnInit() {
        super.ngOnInit();
        // this.isShowInfo = (isShowTabHRM(hrmkey.QuaTrinhDang) && isDuyetLyLichNhanSu) || !isDuyetLyLichNhanSu;
    }
    ...
    removeHandler(dataItem: IQuaTrinhDang) {
        this.selectionIds = [];
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOnOk: () => {
                    this.apiService
                        .delete(UrlConstant.API.HRM_NS_QUA_TRINH_DANG, body)
    ...
    onApprove(flag: boolean) {
        const body: IDuyetThongTinNhanSuModel = {
            ...
            idsNhanSuQuaTrinhDang: this.selectionIds,
        };
        this.opened = true;
    ...
    protected showFormCreateOrUpdate() {
        this.opened = true;
        const windowRef = this.windowService.open({
            title: 'Quá trình Đảng',
            content: FormQuaTrinhDangComponent,
            width: 700,
            top: 10,
            autoFocusedElement: 'body',
        });
    ...
    protected loadItems() {
        this.loading = true;
        this.gridView$ = this.apiService.read(UrlConstant.API.HRM_NS_QUA_TRINH_DANG, this.queryOptions).pipe(
