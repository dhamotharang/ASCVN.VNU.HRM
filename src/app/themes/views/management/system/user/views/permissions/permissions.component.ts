...
} from '@themes/views/management/human-resource/components';
...
export class PermissionsComponent extends BaseCheckPermission implements OnInit, OnDestroy {
    ...
    private loadItems() {
        this.isLoading = true;
        this.gridView$ = this.apiService.post(UrlConstant.API.ACL_PERMISSION + '/GetUserGroupPermissions', this.queryOptions).pipe(
            map(res => {
                if (res.result) {
                    const arrayData = res.result.items;
                    arrayData.forEach(element => {
                        if (element.groupPemissions && element.groupPemissions.length > 0) {
                            element.groupPemission = [];
                            element.groupPemissionId = [];
                            element.groupPemissions.split('|').forEach(element2 => {
