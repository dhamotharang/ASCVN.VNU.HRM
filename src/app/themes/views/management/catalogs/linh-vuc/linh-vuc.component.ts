...
import { IChucVu, ILinhVuc } from '@themes/views/management/catalogs/_models/catalog.model';
...
import { FormLinhVucComponent } from './form-linh-vuc/form-linh-vuc.component';
...
export class LinhVucComponent extends BaseCatalogComponent<IChucVu> implements OnInit, OnDestroy {
    constructor(
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('CATALOG.LINH_VUC.TITLE'),
            content: FormLinhVucComponent,
            width: 550,
            top: 100,
     ...
     * @param dataItem
     */
    removeHandler(dataItem: ILinhVuc) {
        this.selectionIds = [];
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOkType: 'danger',
                nzOnOk: () => {
                    this.apiService.delete(UrlConstant.API.DM_LINH_VUC, body).subscribe(res => {
                        // show notification
    ...
    loadItems() {
        // super.loadItems();
        this.isLoading = true;
        this.gridView$ = this.apiService.read(UrlConstant.API.DM_LINH_VUC + '/List', this.queryOptions).pipe(
            map(res => {
