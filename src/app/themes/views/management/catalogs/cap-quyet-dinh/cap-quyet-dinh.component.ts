...
import { FormCapQuyetDinhComponent } from './form-cap-quyet-dinh/form-cap-quyet-dinh.component';
import { CustomTranslateService } from '@core/services/common/custom-translate.service';

@Component({
    selector: 'app-cap-quyet-dinh',
    templateUrl: './cap-quyet-dinh.component.html',
    styleUrls: ['./cap-quyet-dinh.component.scss'],
})
export class CapQuyetDinhComponent extends BaseCatalogComponent<ICapQuyetDinh> implements OnInit, OnDestroy {
    constructor(
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('CATALOG.CAP_QUYET_DINH.TITLE'),
            content: FormCapQuyetDinhComponent,
            width: 550,
            top: 100,
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOkType: 'danger',
                nzOnOk: () => {
                    this.apiService.delete(UrlConstant.API.DM_CAP_QUYET_DINH, body).subscribe(res => {
                        // reset
                        this.selectionIds = [];
    ...
    protected loadItems() {
        this.isLoading = true;
        this.gridView$ = this.apiService.read(UrlConstant.API.DM_CAP_QUYET_DINH + '/List', this.queryOptions).pipe(
            map(res => {
