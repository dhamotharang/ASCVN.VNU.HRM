...
import { FormKhenThuongComponent } from './form-khen-thuong/form-khen-thuong.component';
...
export class KhenThuongComponent extends BaseCatalogComponent<IKhenThuong> implements OnInit, OnDestroy {
    constructor(
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('CATALOG.KHEN_THUONG.TITLE'),
     ...
     * @param dataItem
     */
    removeHandler(dataItem: IKhenThuong) {
        this.selectionIds = [];
        this.selectionIds.push(dataItem.khenThuongId);
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            const body = {
                khenThuongIds: this.selectionIds,
            };
            this.modal.confirm({
                ...
                nzOkType: 'danger',
                nzOnOk: () => {
                    this.apiService.delete(UrlConstant.API.DM_KHEN_THUONG, body).subscribe(res => {
                        this.selectionIds = [];
                        // show notification
    ...
    protected loadItems() {
        // super.loadItems();
        this.isLoading = true;
        this.gridView$ = this.apiService.read(UrlConstant.API.DM_KHEN_THUONG + '/List', this.queryOptions).pipe(
            map(res => {
