...
import { FormTinhTrangSucKhoeComponent } from './form-tinh-trang-suc-khoe/form-tinh-trang-suc-khoe.component';
...
export class TinhTrangSucKhoeComponent extends BaseCatalogComponent<ITinhTrangSucKhoe> implements OnInit, OnDestroy {
    constructor(
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('CATALOG.TINH_TRANG_SUC_KHOE.TITLE'),
            content: FormTinhTrangSucKhoeComponent,
            width: 550,
            top: 100,
     ...
     * @param dataItem
     */
    removeHandler(dataItem: ITinhTrangSucKhoe) {
        this.selectionIds = [];
    ...
    removeSelectedHandler() {
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOkType: 'danger',
                nzOnOk: () => {
                    this.apiService.delete(UrlConstant.API.DM_TINH_TRANG_SUC_KHOE, body).subscribe(res => {
                        // show notification
    ...
    loadItems() {
        // super.loadItems();
        this.isLoading = true;
        this.gridView$ = this.apiService.read(UrlConstant.API.DM_TINH_TRANG_SUC_KHOE + '/List', this.queryOptions).pipe(
            map(res => {
