...
import { FormLoaiNhanSuComponent } from './form-loai-nhan-su/form-loai-nhan-su.component';
...
export class LoaiNhanSuComponent extends BaseCatalogComponent<ILoaiNhanSu> implements OnInit, OnDestroy {
    constructor(
    ...
    showFormCreateOrUpdate() {
        ...
        const windowRef = this.windowService.open({
            title: this.translate.get('CATALOG.LOAI_NHAN_SU.TITLE'),
            content: FormLoaiNhanSuComponent,
            width: 550,
            top: 100,
     ...
     * @param dataItem
     */
    removeHandler(dataItem: ILoaiNhanSu) { 
        this.selectionIds = [];
        this.selectionIds.push(dataItem.loaiNhanSuId);
    ...
    removeSelectedHandler() { 
        if (this.selectionIds.length > 0) {
            ...
            this.modal.confirm({
                ...
                nzOkType: 'danger',
                nzOnOk: () => {
                    this.apiService.delete(UrlConstant.API.DM_LOAI_NHAN_SU, body).subscribe(res => {
                        // reset
                        this.selectionIds = [];
    ...
    protected loadItems() {
        this.isLoading = true;
        this.gridView$ = this.apiService.read(UrlConstant.API.DM_LOAI_NHAN_SU + '/List', this.queryOptions).pipe(
            map(res => {
